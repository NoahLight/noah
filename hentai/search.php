<?php
  ...
  if (empty($_GET["nx"])){
    $urll = "https://hentaimama.io/?s=".$_GET["q"];
  }else {
    $urll = "https://hentaimama.io/page/".$_GET["nx"]."/?s=".$_GET["q"];
  }
?>
<!DOCTYPE html>
<html lang="en">
  ...
  <div class="container mt-4 mb-4">
    ...
    <div class="hentai mt-4 mb-4">
      <div class="list-hentai">
        <div class="row mt-2">
          <?php
            $html = str_get_html(file_get_contents($urll));
            foreach ($html->find("div[class=listupd]") as $cek){
              $chek = strpos($cek, "Not Found");
              if ($chek){
                header("Location: NotFound.php");
              }
            }
      ...
      <?php
        foreach ($html->find("div[class=pagination]") as $got){
          foreach ($html->find("a[class=prev page-numbers]") as $gui){
            $agiy = str_replace(array("https://hentaimama.io/page/","/?s=".$_GET["q"]), array("",""), $gui->href);
            $hox = $_GET["q"];
            ?>
          ...
          foreach($got->find("a[class=next page-numbers]") as $gut){
            $goy = str_replace(array("https://hentaimama.io/page/","/?s=".$_GET["q"]), array("",""), $gut->href);
            $hox = $_GET["q"];
            ?>
